...
var getBlocks = function (dest, dir, content) {
  ...
  lines.forEach(function (l) {
    ...
    var protocalRelativeURL = false;

    // discard empty lines
    if (build) {
      block = true;
      // support Protocol-relative URL with leading double slashes
      // Handle absolute path (i.e. with respect to the server root)
      if (build[3].substr(0,2) === '//') {
        protocalRelativeURL = true;
        build[3] = build[3].substr(2);
      } else if (build[3][0] === '/') {
        startFromRoot = true;
      ...
      last = {
        ...
        protocalRelativeURL: protocalRelativeURL,
        indent: indent,
        src: [],
        raw: []
      };
...
HTMLProcessor.prototype.replaceWith = function replaceWith(block) {
  ...
  if (block.protocalRelativeURL) {
    dest = '//' + dest;
  } else if (block.startFromRoot) {
    dest = '/' + dest;
  }
